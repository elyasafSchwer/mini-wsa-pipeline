...
/**
 * Integration test for the samples query path, run under the fully in-memory
 * {@code standalone} profile — no Elasticsearch, no Docker. The wired {@link SamplesService}
 * is the in-memory implementation; fixtures are written through the real
 * {@link EventStorageService} → store path so the exercised document mapping is the one the
 * app actually uses. The shared {@link InMemoryEventStore} is cleared before each test for
 * isolation.
 */
@SpringBootTest
@ActiveProfiles("standalone")
class SamplesQueryTests {

    private static final long CONFIG = 910_000_000L;
